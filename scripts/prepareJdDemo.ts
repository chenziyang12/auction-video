...
const main=async()=>{console.log('开始获取京东拍卖列表');
  ...
  for(const product of products){try{await downloadJdImage(product.remoteImage,product.id); const {remoteImage: _remoteImage,...local}=product; downloaded.push(local);}catch(error){console.warn(`图片下载失败 ${product.id}:`,error instanceof Error?error.message:error);}}
