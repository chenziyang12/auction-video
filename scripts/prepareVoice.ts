...
const loadItems=async():Promise<AuctionItem[]>=>{if(mode==='demo')return demoProperties;const file=path.resolve('src','data','jd-demo.json');try{await access(file);}catch{throw new Error('请先执行 npm run fetch:jd');}const value:unknown=JSON.parse(await readFile(file,'utf8'));if(!Array.isArray(value)||!value.length)throw new Error('请先执行 npm run fetch:jd');return value as AuctionItem[];};
const provider=new EdgeTtsProvider(); const cacheDir=path.resolve('public','generated','tts');
